...
const VerifyScreen = ({ navigation }) => {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (response.ok && response.token) {
        ...
        // Trường hợp backend không trả về token dù không báo lỗi
        throw new Error(response.message || 'Xác thực thành công nhưng không nhận được token.');
      }
    } catch (error) {
  ...
  const handleResendCode = async () => {
    // 1. Điều kiện "phòng vệ": Không làm gì nếu đang đếm ngược hoặc đang trong quá trình gửi lại.
    // Điều này ngăn người dùng nhấn nút liên tục.
    if (countdown > 0 || isResending) return;
    // 2. Báo cho giao diện biết rằng quá trình gửi lại đã bắt đầu.
    // Giao diện sẽ hiển thị "Đang gửi..."
    setIsResending(true);

    try {
      // Gọi hàm requestOTP với số điện thoại đã được truyền từ màn hình trước.
      await requestOTP(phone);
      // Nếu API thành công:
      Alert.alert('Thành công', 'Mã xác thực mới đã được gửi lại!');
      // Reset bộ đếm ngược về 60 giây.
      setCountdown(60);
    } catch (error) {
      // Nếu API thất bại (ví dụ: do backend giới hạn số lần gửi):
      // Lấy thông báo lỗi từ server hoặc một thông báo mặc định.
      const errorMessage = error.response?.data?.message || error.message;
      Alert.alert('Gửi lại thất bại', errorMessage);
    } finally {
      // 4. Khối này LUÔN LUÔN chạy, dù API thành công hay thất bại.
      // Đảm bảo rằng trạng thái loading được tắt để người dùng có thể thử lại sau đó.
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        >
          <Image
            source={require('@assets/images/Exit_black.png')}
            style={styles.icon}
          />
        </TouchableOpacity>
      </View>
